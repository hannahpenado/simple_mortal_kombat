...
class Character:
  ...
  def __init__(self, player_num):
    self.character_list = ['Kitana', 'Frost', 'Shao Kahn', 'Scorpion']
    ### moves damage 7, 9, 14 in order of list index 0, 1, 2
    self.attack_moves = {'Kitana': ['Low Slice', 'Full Slice', 'Back Throw'], 'Frost': ['Blade Lunge', 'Ice Spike', 'Frosted Uppercut'], 'Shao Kahn': ['Hammer Slammer', 'Deadly Swipe', 'Rising Wrath'], 'Scorpion': ['Flip Kick', 'Spear Slice', 'Rising Spear']}
    self.character = ''
    self.final_kill_move_dict = {'Kitana': ['Gore Nado', 'Royal Execution'], 'Frost': ['Ice Sculpture', 'The Cyber Initiative'], 'Shao Kahn': ['YOU SUCK', 'Head Kabob'], 'Scorpion': ['Get Over Here', 'Ashes']}
...
# Allows user to select character from a list of available characters and will remove that character after chosen
  def choose_character(self):
    ...
    for character in player1.character_list:
      ...
      num += 1
    choice = int(input('Select a number for a player and press enter to select.')) - 1
  ...
  def choose_final_kill(self):
    if self.character in self.final_kill_move_dict:
      ...
      for move in final_kill_options:
        print(str(num) + ' ' + move) 
        num += 1
      choice = int(input('Type corresponding number for final kill choice.')) - 1
...
## Allows player to attack other player and decrease their health by an amount multiplied by the attack level
  def attack_other_player(self, other_player):
    ...
    while attack not in range(1, 4):
      attack = input('Please enter the attack number 1-3\n')
    self.attacks += 1
    if self.enhancement == 'Extra Strength':
      if attack == 1:
        other_player.health -= 12
      elif attack == 2:
        other_player.health -= 14
      else:
        other_player.health -= 19
    else:
      if attack == 1:
        other_player.health -= 7
      elif attack == 2:
        other_player.health -= 9
      else:
          other_player.health -= 14
      health1 = self.health
      health2 = other_player.health
      attack_name = move_list[attack - 1]
      print('{char1} uses {attack} on {char2}'.format(char1 = character, attack = attack_name, char2 = character2))
    if other_player.health <= 30:
      print('{char} has final kill move ready for use'.format(char = character))
  ...
  def block_attack(self, other_player):
    character = self.character
    character2 = other_player.character
    print('''{char1} blocks {char2}.'''.format(char1 = character, char2 = character2))
...
### Allows player to use final kill move if otehr players health is 30 or less
  def final_kill(self, other_player):
    ...
    if other_player.health <= 30:
      ...
      print('{char1} uses {kill} on {char2}, {char1} wins. Flawless Victory!'.format(char1 = self.character, char2 = other_player.character, kill = final_kill))
    else:
...
print('\nNow we will begin our fight between {char1} and {char2}'.format(char1 = player1.character, char2 = player2.character))
